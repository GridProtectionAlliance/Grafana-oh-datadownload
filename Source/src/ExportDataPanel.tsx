...
export const ExportDataPanel: React.FC<Props> = ({ options, data, width, height }) => {
  ...
  React.useEffect(() => {
    ...
    const link = options.Link + 'ExportDataHandler.ashx?PointIDs=' + pointIDs.join(",") +
            "&StartTime=" + moment.utc(startTime).format("MM/DD/YYYY HH:mm:ss.000000") + "&EndTime=" + moment.utc(endTime).format("MM/DD/YYYY HH:mm:ss.000000") +
            "&FrameRate=" + framerate.toString() + "&AlignTimestamps=" + ((options.AlignTS) ? "true" : "false") +
            "&MissingAsNaNParam=" + ((options.ExportNaN) ? "true" : "false") +
            "&FillMissingTimestamps=" + ((options.MissingTS) ? "true" : "false") +
            "&InstanceName=" + "PPA" +
            "&TSSnap=" + formatTSSnap().toString() +
            "&TSTolerance=" + tolerance.toString();

      setFileLink(link);
  }, [options, startTime, endTime, pointIDs])

  function formatTSSnap() {
  ...
  React.useEffect(() => {
    ...
    setPointIDs([]);
      ...
      data.series.forEach(element => {
        ...
        getBackendSrv().post(options.Link + 'api/grafana' + '/GetMetadata',dat).then((d) => {
  ...
  return (
    <div className={cx(styles.wrapper, css`
      ...
      <div className="button" style={{backgroundColor: options.Color,
        height: 'calc(100% - 5px)',
        width: 'calc(100% - 5px)',
        margin: '5px',
        textAlign: 'center',
